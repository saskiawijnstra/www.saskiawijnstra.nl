$(function() {
    ...
    */
    function slide(event) {
        if (sliding > 1) event.preventDefault();
      if (event.originalEvent.touches)
        event = event.originalEvent.touches[0];
      // Distance of slide.
      ...
      if (sliding == 1 && (deltaSlide > startSlidingOffset || deltaSlide < -startSlidingOffset)) {
    ...
    */
    function slideEnd(event) {
      if (sliding == 2){
        ...
        currentSlide = Math.min(Math.max(currentSlide, 0), slideCount - 2);
        // Since in this example slide is full viewport width offset can be calculated according to it.
        pixelOffset = currentSlide * -($('.slide').width() + 40);
